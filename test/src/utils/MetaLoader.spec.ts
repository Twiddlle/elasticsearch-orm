...
describe('meta loader', () => {
  ...
  it('should load metadata of same class name from different file', () => {
    ...
    expect(metadata.entity.alisases[0]).toBe('test_alias_read_2');
    expect(metadata.entity.alisases[1]).toBe('test_alias_write_2');

    expect(metadata.props[0].type).toBe('integer');
    expect(metadata.props[0].name).toBe('Foo_2');
    expect(metadata.props[0].fieldOptions.boost).toBe(10);
    expect(metadata.props[1].type).toBe('boolean');
    expect(metadata.props[1].name).toBe('bar2');
    expect(metadata.props[2].type).toBe('geo_point');
    expect(metadata.props[2].name).toBe('geoPoint2');
  });
});
